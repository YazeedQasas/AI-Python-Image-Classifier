...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(file_list), 1):
       ...
       if file_list[idx][0] != ".":
           ...
           #          filename in the file_list list to extract the dog breed 
           #          name from the filename. Recall that each filename can be
           #          accessed by in_files[idx]. Be certain to place the 
           #          extracted dog breed name in the variable pet_label 
           #          that's created as an empty string ABOVE
           pet_label_section = file_list[idx].lower().split("_")
           ...
           if file_list[idx] not in results_dic:
              results_dic[file_list[idx]] = [pet_label]
