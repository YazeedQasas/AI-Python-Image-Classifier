    ...
    for idx in range(0, len(in_files), 1):
       ...
       if in_files[idx][0] != ".":
           ...
           # Creates temporary label variable to hold pet label name extracted 
            ...
            #Extract the labels for the pets from filename. (1)
            #Remove extension for file, interferring numbers, converted to lowercase. (2)
            #Split the code by underscores and joining with spaces. (3)

            pet_label_section = in_files[idx].lower().split('_')
            pet_label = " ".join([word for word in pet_label_section if word.isalpha()]).strip() 
